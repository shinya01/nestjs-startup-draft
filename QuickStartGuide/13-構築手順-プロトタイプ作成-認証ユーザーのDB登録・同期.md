...
## 💡 補足：Auth0 を利用する場合の同期ロジック

Auth0 の場合は AWS SDK の代わりに `HttpService` (axios) を使用して標準的な OAuth 2.0 UserInfo エンドポイントへアクセスします。

### 1. パッケージのインストール (Auh0用)

```bash
npm install @nestjs/axios axios
```

### 2. `Auth0Strategy` での実装例

Auth0 の `issuer` URL に `/userinfo` を連結してリクエストを送信します。
...
#### `src/auth/auth.module.ts`(Auth0用追加)
...
@Module({
  imports: [
    ...
    UserModule,
    HttpModule, // Auth0 の HttpService 利用に必要
  ],
  providers: [CognitoStrategy, Auth0Strategy, CognitoAuthGuard],
  exports: [PassportModule, CognitoAuthGuard],
})
export class AuthModule {}
```
...
- [Auth0 Docs: /userinfo Endpoint](https://auth0.com/docs/api/authentication#get-user-info)
